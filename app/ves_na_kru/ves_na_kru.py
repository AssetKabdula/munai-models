...
def load_models_and_scalers():
    ...
    nn_model = tf.keras.models.load_model(f'{BASE_DIR}/nn_model_ves_na_kru.h5')
    xgb_model = joblib.load(f'{BASE_DIR}/xgb_model_ves_na_kru.pkl')
    x_scaler = joblib.load(f'{BASE_DIR}/x_scaler_ves_na_kru.pkl')
    y_scaler = joblib.load(f'{BASE_DIR}/y_scaler_ves_na_kru.pkl')
...
def get_output_columns():
    ...
    return ['Грузоподъёмность вышки', 'Бурение ротором', 'Подъём', 
                'Спуск', 'Бурение ГЗД', 'Мин. вес до спирального изгиба (спуск)',
                'Макс. вес до предела текучести (подъём)'] 


# Make prediction with the model
def make_prediction_ves_na_kru(input_data):
